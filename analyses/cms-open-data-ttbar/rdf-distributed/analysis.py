...
PARSER.add_argument("--cluster-type", "-c", help="The type of Dask cluster deployment.",
                    default="local", choices=["local", "ssh"])
PARSER.add_argument("--nodes", help="String containing the list of hostnames to be used. Useful only in pair with 'ssh'"
                                    " choice for the '--cluster-type' parameter",
...
def create_connection(nodes: str, ncores: int, cluster_type: str) -> Client:
    ...
    # Add more cluster types here to accomodate different deployments
        ...
        raise ValueError(f"Unexpected cluster type '{cluster_type}'. Acceptable values are ['local', 'ssh'].")
...
class TtbarAnalysis(dict):
    ...
    def fill(self, process: str, variation: str, connection: Client):
    ...
    def Fill(self, connection: Client):
        self.hist = []
        for process in self:
            for variation in self.input_data[process]:
                self.fill(process, variation, connection)
...
def main():
    ...
    with create_connection(ARGS.nodes, ARGS.ncores, ARGS.cluster_type) as conn:
